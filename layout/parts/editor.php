...
<form name="tx_editor_form" id="tx_editor_form" action="/admin/modules/editor_post.php" method="post" enctype="multipart/form-data" accept-charset="utf-8">
<div id="thumbnail">
make popup image : 
	   <input type="file" name="thumb" id="thumb"/>
	   </div>

	   <div style="margin:1em 0;line-height:1em;vertical-align:middle">title:  <input id="title" class="input-xxlarge" type="text" name="title" size="500"/></div>
		<!-- 에디터 컨테이너 시작 -->
...
$.ajax("event_thumb.php");
		// sample : validate that content exists
		var validator = new Trex.Validator();
		var thumbName = document.getElementById ("thumb");
		...
		if (!validator.exists(content)) {
		  ...
		  alert("Please upload event popup image.");
		  return false;
		}
	...
	function setForm(editor) {
	  ...
	  //formdata.append("file", file);
           // $.ajax({url: "event_thumb.php", type: "POST", data: "darkzin:12312", processData: false, contentType: "application/x-www-form-urlencoded"});
		//url: "/admin/modules/event_thumb.php",
		//type: "POST",
		//data: "darkzin=1231",
		//processData: false,
		//contentType: false,
	...
	input.type = 'hidden';
	input.name = 'title';
	input.value = $('#title').val();  // 예에서는 이미지경로만 받아서 사용
...
<div style="text-align: center"><button class="btn btn-large btn-primary" onclick='saveContent()'>submit contents</button></div>
<!-- End: Saving Contents -->
...
<!--<div style="text-align: center"><button onclick='loadContent()'>SAMPLE - load contents to editor</button></div>-->
